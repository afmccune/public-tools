	...
	<body>
        <div id="outer">
			...
			foreach (new DirectoryIterator("./html/") as $fn) {
				if (preg_match('/[0-9]{1,2}.[0-9]{1}[-a-z0-9]{0,3}.toc.html/', $fn->getFilename())) {
					...
					if(showable($fn_t['volNum'], $fn_t['issueShort'])) {
						...
						$fn_t['img'] = '<img width="'.$thumbwidth.'" src="https://upload.wikimedia.org/wikipedia/commons/'.$HTMLsrc[0].'" />';
			...
			for ($i=0; $i<count($docsHtml); $i++) {
				...
				print '<div class="issue html-issue '.$docsHtml[$i]['decade'].' vol'.$docsHtml[$i]['volNum'].' issue'.$docsHtml[$i]['issueShort'].'"><a class="issue-link" href="hdoc.php?file='.$docsHtml[$i]['file'].'"><span class="cover-thumb-holder">'.$docsHtml[$i]['img'].'</span><span class="issue-link-heading">Volume '.$docsHtml[$i]['volNum'].' &middot; Issue '.$docsHtml[$i]['issueNum'].'<br/>('.$docsHtml[$i]['date'].')</span></a></div>';
			}
			...
			foreach (new DirectoryIterator("./docs/") as $fn) {
				if (preg_match('/[0-9]{1,2}.[0-9]{1}[-a-z0-9]{0,3}.toc.xml/', $fn->getFilename())) {
					...
					if(showable($fn_t['volNum'], $fn_t['issueShort'])) {
						...
						$fn_t['img'] = '<img width="'.$thumbwidth.'" src="https://upload.wikimedia.org/wikipedia/commons/'.$XMLimg[0].'"/>';
			...
			for ($i=0; $i<count($docsXml); $i++) {
				...
				print '<div class="issue xml-issue '.$docsXml[$i]['decade'].' issue'.$docsXml[$i]['issueShort'].'"><a class="issue-link" href="xdoc.php?file='.$docsXml[$i]['file'].'"><span class="cover-thumb-holder">'.$docsXml[$i]['img'].'</span><span class="issue-link-heading">Volume '.$docsXml[$i]['volNum'].' &middot; Issue '.$docsXml[$i]['issueNum'].'<br/>('.$docsXml[$i]['date'].')</span></a></div>';
