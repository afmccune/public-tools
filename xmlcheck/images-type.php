       ...
       <div id="outer">
				...
				<div id="content-inner">
					<div id="issue-heading">
						<div class="issue-heading-inner">
							<h1>Images with nonstandard type attribute</h1>
						</div>
					</div>
					<div id="main">
			...
			foreach (new DirectoryIterator($dir) as $fn) {
				if (preg_match('/[0-9]{1,2}.[0-9]{1}[-a-z0-9]{0,3}.[-a-z0-9]{1,20}.xml/', $fn->getFilename())) {
					...
					$fn_t['imgType'] = $FullXML->xpath('//text//figure/@type'); // array
					...
					if(count($fn_t['imgType']) > 0) {
						foreach($fn_t['imgType'] as $type) {
							if ($type != 'reviewed-cover') {
								$fn_t['errors'][] = 'Nonstandard image type: '.$type;
							}
						}
					} else {
						//
