				...
				<div id="content-inner">
					<div id="issue-heading">
						<div class="issue-heading-inner">
							<h1>Word List: Seek (code - what files lack a string)</h1>
						</div>
					</div>
					<div id="main">
						<div id="allIssues">
								...
								if(in_array($fn_t['file'], $scannedFiles)) {
									...
									foreach($seek as $keyword) {
										if(mb_ereg_match('.*'.$keyword.'', $fn_t['text'])) {
											echo '<p>'.$fn_t['fn'].' contains: '.$keyword.'</p>';
										} else {
											// if does not contain keyword
											echo '<h4>'.$fn_t['fn'].' does not contain: '.$keyword.'</h4>';
											// indicate file in which it does not appear
											$masterList[] = '['.$fn_t['file'].'] LACKS '.$keyword;
										}
