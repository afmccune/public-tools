						...
						foreach (new DirectoryIterator("../../bq/docs/") as $fn) {
							if (preg_match('/.xml/', $fn->getFilename()) && $fn->getFilename() != "Emend.xml") {
								...
								if(in_array($fn_t['file'], $scannedFiles)) {
									...
									$fn_t['text'] = str_replace($nl, ' '.$nl.' ', $fn_t['text']);
									...
									$hugeText = $hugeText.$nl.$fn_t['text'];
									file_put_contents('wordlist/_huge-text-'.$start.'.txt', $hugeText);
									echo '<h4>'.$fn_t['fn'].' added to hugeText.</h4>';
