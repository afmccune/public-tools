...
// new diff
	...
	if ( isset($_GET['file']) ) {
		$from_text = file_get_contents('../bq-xmltransform/old/'.$_GET['file'].'.xml');
		$to_text = file_get_contents('../bq-xmltransform/new/'.$_GET['file'].'.xml');		
	}
	

	// limit input
	$from_text = substr($from_text, 0, 1024*400);
	$to_text = substr($to_text, 0, 1024*400);
